  ...
  return (
    <>
      ...
      <main className="landing">
        ...
        <section className="section" id="products"><div className="sectionHeading"><span className="eyebrow">Retail essentials</span><h2>Food, shampoo, and trusted care products.</h2><p>Reserve available cat and dog essentials online, choose Cash, GCash, or Maya, then complete payment and pickup at the physical store.</p></div>{products.length ? <div className="publicProductGrid">{products.map((product) => <article key={product.productId}><img className="productImage" src={productImageSrc(product.image, product.productName, product.category, product.productId)} alt={product.productName} loading="lazy" /><small>{product.productGroup} · {product.category}</small><h3>{product.productName}</h3><p>{product.description || "Available for store pickup."}</p><div><b>{money(product.price)}</b><span>{product.isActive && product.stockQuantity > 0 ? `${product.stockQuantity} available` : "Unavailable"}</span></div></article>)}</div> : <div className="dashboardEmpty"><b>Catalog temporarily unavailable</b><small>Log in later to view current store inventory.</small></div>}<div className="catalogAction"><Link className="primaryButton" href="/client/login">Reserve products</Link></div></section>
